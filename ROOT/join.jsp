...
<script type="text/javascript">
	...
	function pwCheck(){
		...
		if(document.getElementById("userpwinput").value != document.getElementById("userpwreinput").value){	// 입력된 두 비밀번호가 서로 다를 경우
	...
	function pwValidCheck(){
		...
		/*	비밀번호에 특수문자 포함여부 체크를 위해서 삽입한 변수(현재 미구현)		*/
		/*	var SC = ["!","@","#","$","%"];						*/
        /*	var check_SC = 0;									*/
	...
	function email_change() {
		if (document.joinpol.email.options[document.joinpol.email.selectedIndex].value == '0') {
			...
			document.joinpol.user_email2.readOnly = false;	// "직접 입력"을 제외한 나머지를 선택시 input박스를 변경할 수 없도록
			document.joinpol.user_email2.value = "";
